...
def addToQueue(mat, grid, queue, d, old_dir, i, j, new_dir):
...
def minimumMoves(grid, startX, startY, goalX, goalY):
    ...
    while i != goalX or j != goalY:
        ...
        while p < n and grid[p][j] == '.':
            addToQueue(mat, grid, queue, d, drn, p, j, 'U')
            p += 1
        p = j + 1
        while p < n and grid[i][p] == '.':
            addToQueue(mat, grid, queue, d, drn, i, p, 'L')
            p += 1
        p = i - 1
        while p >= 0 and grid[p][j] == '.':
            addToQueue(mat, grid, queue, d, drn, p, j, 'D')
            p -= 1
        p = j - 1
        while p >= 0 and grid[i][p] == '.':
            addToQueue(mat, grid, queue, d, drn, i, p, 'R')
            p -= 1
